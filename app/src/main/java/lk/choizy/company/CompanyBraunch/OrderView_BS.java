...
public class OrderView_BS extends BottomSheetDialogFragment {
    ...
    public View onCreateView(@NonNull @NotNull LayoutInflater inflater, @Nullable @org.jetbrains.annotations.Nullable ViewGroup container, @Nullable @org.jetbrains.annotations.Nullable Bundle savedInstanceState) {
        ...
        studentId.setText(orders.getStudentID());
    ...
    public void setupBtn(){
        if(orders.getOrderStatus().equals("Pending")&&(orders.getOrderType().equals("Delivery")||orders.getOrderType().equals("orderAdvance"))){
            orderFisnhBtn.setText("Order Ready");
            orderFisnhBtn.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    if(listener != null){
                        orders.setOrderStatus("Ready");
                        listener.onOrderUpdate(orders);
                        setupBtn();
                    }
                }
            });
        }else{
            orderFisnhBtn.setText("Order Finish");
            orderFisnhBtn.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    if(listener != null){
                        orders.setOrderStatus("Ready");
                        orders.setPaymentDone(true);
                        listener.onOrderFinish(orders);
                        dismiss();
                    }
                }
            });
    ...
    public OrderView_BS(Orders orders) {
        this.orders = orders;
    }

    public interface onOrderListener{
        void onOrderUpdate(Orders order);
        void onOrderFinish(Orders order);
    }
